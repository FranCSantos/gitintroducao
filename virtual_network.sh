...
ip netns add blue
ip netns add red
ip link add vethr type veth peer name veth0 netns red
...
ip netns exec red ip addr add 10.0.0.2/28 dev veth0
...
ip route
ip netns exec red ip route add default via 10.0.0.1 dev veth0
