...
ip netns add yellow
ip link add vethr type veth peer name veth1 netns yellow
...
ip netns exec yellow ip addr add 10.0.0.2/28 dev veth1
...
ip netns exec yellow ip link set veth1 up
...
ip netns exec yellow ip route add default via 10.0.0.1 dev veth1
